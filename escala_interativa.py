...
with st.form("form_participantes"):
    for i in range(int(num_pessoas)):
        ...
        turnos_semanais = col2.number_input(f"Turnos para {nome or 'participante'}", min_value=1, max_value=50, value=1, key=f"turno_{i}")
        if nome:
            nomes.append(nome)
...
if submitted:
    dias = ['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta'][:dias_semana]
